	...
	public void forgotPass() {
		try {
			...
			if(login != null) {
				...
				setPassword(password = login.getPassword().trim());
				FacesContext.getCurrentInstance().addMessage(null,
				...
				if(loginPostgres != null) {
					...
					setPassword(password = loginPostgres.getPassword().trim());
					FacesContext.getCurrentInstance().addMessage(null,
							new FacesMessage(FacesMessage.SEVERITY_INFO,
									properties.getProperty("passMess"), properties.getProperty("passRecupera").
									concat("TU CONTRASEÑA ES: ").concat(password)));
